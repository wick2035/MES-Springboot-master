...
<div class="wf-shell">
    <div class="wf-hero wf-hero-command">
        <div>
            <h1 class="wf-title">流程定义管理</h1>
            <div class="wf-subtitle">在这里新增流程表单，绑定生产订单审批流程、URL 表单地址、安全事件和任务按钮选项。</div>
        </div>
...
<script>
    layui.use(['form', 'table', 'layer', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url:'${request.contextPath}/workflow/definition/page',
            cols:[[
        ...
        table.on('tool(js-record-table-filter)', function(obj){
            ...
            if (obj.event === 'delete') {
                layer.confirm('确认删除该流程表单吗？', function(index){
                    spUtil.ajax({url:'${request.contextPath}/workflow/definition/delete', type:'POST', serializable:false, data:{id:obj.data.id}, success:function(){ tableIns.reload(); layer.close(index); }});
                });
            }
        });
        function openEdit(id) {
            spLayer.open({
                ...
                area:['980px','760px'],
                spWhere:id ? {id:id} : {},
                content:'${request.contextPath}/workflow/definition/add-or-update-ui'
            });
        }
